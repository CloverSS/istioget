...
public class GraphAnalyzeHelper {
				...
				for (int i = 0; i < nodes.length(); i++) {
					...
					if (data.has("id")) {
						...
						if (data.has("service")) {
							...
							Double throughput = FindLatency.SelectQps(BaseConf.istio_ip, BaseConf.prom_port, namespace, service,
									"1m");
							node.setThroughput(throughput);
							//System.out.println("service: "+service+" throughput: "+throughput);
							...
							if(data.has("nodeType")&&data.has("workload")){
								if(data.getString("nodeType").equals("unknown")&&data.getString("workload").equals("unknown")) {
									node.setService("unknown");
									this.unknown_id = id;
								}
							}
							NodeMap.put(id, node);
				...
				for (int i = 0; i < edges.length(); i++) {
					...
					if (data.has("id")) {
						...
						if (data.has("source")) {
							...
							if(source.equals(unknown_id)) {
								NodeMap.get(target).setRoot();
							}
						}
						if (data.has("traffic")) {
							...
							if (traffic.has("responses")) {
								...
								if (it.hasNext()) {
									...
									if (responses.getJSONObject(code).getJSONObject("flags").has("-")) {
										Double percentres = responses.getJSONObject(code).getJSONObject("flags")
												.getDouble("-");
	...
	private void transEdgeToNode() {
		for (Edge edge : EdgeList) {
			NodeMap.get(edge.getsource()).addDownstream(edge.gettarget(), getServiceName(edge.gettarget()),
